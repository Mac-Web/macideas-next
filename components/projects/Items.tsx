...
function Items({ open, search, handleOpen, sort }: ItemsProps) {
  const router = useRouter();

  return (
    <>
      {(sortList(open.folders, search, sort) as FolderType[]).map((folder) => {
        return (
          <div
            key={folder.id}
      ...
      {(sortList(open.tasks, search, sort, true) as Task[]).map((task) => {
        return (
          <div
            key={task.id}
            onDoubleClick={() =>
      ...
      {(sortList(open.notes, search, sort) as Note[]).map((note) => {
        return (
          <div
            key={note.id}
